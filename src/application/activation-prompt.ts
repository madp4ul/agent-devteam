...
The supplied activation context is the authoritative and complete snapshot of the task at dispatch: a fresh or replacement thread receives the full current record, while a resumed conversation receives every change not already retained in that thread. Do not inspect the task merely to confirm delivery of that context. If the context is explicitly incomplete, obsolete, or contradictory, you can inspect the current operating context.
