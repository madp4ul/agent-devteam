...
test("a follow-up durably owns its attachments within one agent conversation", async (t) => {
  ...
  const created = application.createTask({
    ...
    idempotencyKey: "create-attachment-conversation",
  });
  ...
  const uploaded = await application.createConversationUpload({
    ...
    content: Readable.from([Buffer.from("durable spreadsheet evidence")]),
  });
  assert.equal(uploaded.accepted, true);
  if (!uploaded.accepted) return;
  assert.equal(uploaded.upload.fileName, "evidence.xlsx");
  ...
  const unrelatedPending = await application.createConversationUpload({
    taskId: created.task.id,
    conversationId,
    fileName: "keep-on-replay.txt",
    mediaType: "text/plain",
    content: Readable.from([Buffer.from("still pending")]),
  });
  assert.equal(unrelatedPending.accepted, true);
  if (!unrelatedPending.accepted) return;
  assert.deepEqual(application.continueAgentConversation({
    taskId: created.task.id,
    conversationId,
    body: "A changed retry payload must not create a second message.",
    attachmentIds: [unrelatedPending.upload.id],
    actor: { kind: "user", id: "paul" },
    idempotencyKey: "continue-with-attachment",
  }), continued);
  assert.equal(application.removeConversationUpload({
    taskId: created.task.id,
    conversationId,
    uploadId: unrelatedPending.upload.id,
  }), true);
  const attachmentRequest = await runtime.waitForRequest(2);
  assert.deepEqual((attachmentRequest?.attachments ?? []).map(({ id, fileName, currentMessage }) => ({
    id,
    fileName,
    currentMessage,
  })), [{ id: uploaded.upload.id, fileName: "evidence.xlsx", currentMessage: true }]);
  assert.match(attachmentRequest?.attachments?.[0]?.path ?? "", /evidence\.xlsx$/);
  ...
  assert.deepEqual(message.message.attachments, continued.message.attachments);
  ...
  for await (const chunk of downloaded.content) downloadedChunks.push(Buffer.from(chunk));
  assert.equal(Buffer.concat(downloadedChunks).toString("utf8"), "durable spreadsheet evidence");

  const otherTask = restarted.createTask({
    boardId: "delivery",
    columnId: "implementation",
  ...
  assert.deepEqual(restarted.readConversationAttachment({
  ...
  for (let index = 0; index < 19; index += 1) {
    pendingUploads.push(await restarted.createConversationUpload({
      taskId: created.task.id,
      conversationId,
  ...
  assert.equal(pendingUploads.every(({ accepted }) => accepted), true);
  const competingUploads = await Promise.all([20, 21].map((number) => restarted.createConversationUpload({
    taskId: created.task.id,
    conversationId,
    fileName: `pending-${number}.txt`,
    mediaType: "text/plain",
    content: Readable.from([]),
  })));
  assert.equal(competingUploads.filter(({ accepted }) => accepted).length, 1);
  assert.deepEqual(competingUploads.find(({ accepted }) => !accepted), {
    accepted: false,
