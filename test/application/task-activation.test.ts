...
test("a same-agent column-entry run moving to another watched column still queues that entry", async (t) => {
  const fixture = await createResponsibilityClaimFixture("column-entry-counterexample");
...
class ControlledRetryClock implements AutomationClock {
  #now: Date;
...
async function createResponsibilityClaimFixture(name: string): ReturnType<typeof createFixture> {
