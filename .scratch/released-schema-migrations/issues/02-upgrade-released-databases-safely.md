...
- [ ] Startup accepts an existing database only when its complete ledger is an
...
- [ ] Gaps, reordered or altered IDs, malformed ledger state, and unknown future
...
- [ ] Before applying pending migrations, startup creates one database-only
...
- [ ] Backup coverage proves committed data still resident in WAL is present and
  readable in the independently opened pre-upgrade backup.
- [ ] The complete pending migration sequence and its ledger entries execute in
...
- [ ] Startup verifies the resulting expected schema, SQLite integrity, and
...
- [ ] Representative populated released-schema fixtures prove direct and
...
- [ ] The fixture strategy can exercise a multi-version migration chain without
...
- [ ] A deliberately injected late failure proves earlier pending steps do not
  ...
  dispatch occurs.
- [ ] Migration is complete before process application, archival recovery,
  task-workspace consistency recovery, host-stop attempt recovery, board
  mutation, or automation dispatch can run.
- [ ] Startup diagnostics distinguish incompatible history, backup failure,
  ...
  corrective action.
- [ ] Backup and restore guidance explains automatic migration backups, manual
  ...
  operational backup.
- [ ] Keep migrations, fixtures, and the checked-in schema snapshot organized
...
- [ ] After tickets 01 and 02 satisfy their acceptance criteria, resolve source
...
- [ ] Run typechecking, focused migration/restart/recovery tests, the complete
