...
- [ ] Each process agent may declare optional `model` and `reasoningEffort`
...
- [ ] Schema validation rejects empty model identifiers and unsupported
...
- [ ] Per-agent model selection changes neither role instructions, coordination
  tools, sandbox mode, approval policy, nor the shared user-controlled
  permission boundary.
- [ ] The software-delivery example explicitly configures every agent initially
  with `gpt-5.6-sol` and `medium`, matching the current behavior while making
  role-by-role changes discoverable and reviewable.
- [ ] Schema, fingerprint, application, runtime-adapter, and controlled SDK
